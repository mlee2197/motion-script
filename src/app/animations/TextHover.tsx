      ...
      <div className="relative h-10 w-full text-center overflow-hidden group">
        <p className="absolute top-0 w-full mx-auto font-shrikhand text-4xl">
          {WORD.map((letter, index) => (
            <span key={letter} className="inline-block transition-transform duration-300 group-hover:translate-y-full" style={{ transitionDelay: `${index * 25}ms` }}>
              {letter}
            </span>
          ))}
        </p>
        <p className="absolute -top-full w-full mx-auto font-shrikhand text-4xl">
          {WORD.map((letter, index) => (
            <span key={letter} className="inline-block transition-transform duration-300 group-hover:translate-y-full text-pink-300" style={{ transitionDelay: `${index * 25}ms` }}>
              {letter}
            </span>
          ))}
        </p>
